...
def match_deployment_to_node(deployments,cumulus):
    ...
    for d in deployments:
        ...
        if not registered:
            ...
            latlng = validate_coordinates(clean_coordinates(
                d["latitude"],d["longitude"]),cumulus["geometry"])
            ...
            if "individualsFilter" in cumulus.keys() or "devicesFilter" in cumulus.keys():
                m_deployment.update({
                    "cumulus_id": cumulus["id"]
                })
            ...
            m_deployment.update({
                ...
                "altitude": match["location"]["coordinates"][2] if latlng else "null"
            })
